...
struct RolloutTests {
    ...
    @Test func rolloutFlagResolution() {
        ...
        let store = FeatureFlagStore(
            ...
            strategies: [
                RolloutStrategy(
                    ...
                    rollouts: ["test.full-rollout": RolloutDefinition(percentage: 100)]
                )
            ]
        )
        ...
        #expect(store.isEnabled("test.full-rollout") == true)
    }
    ...
    @Test func zeroRolloutUsesDefault() {
        ...
        let store = FeatureFlagStore(
            ...
            strategies: [
                RolloutStrategy(
                    enrollment: enrollment,
                    rollouts: ["test.zero-rollout": RolloutDefinition(percentage: 0)]
                )
            ]
        )
        ...
        #expect(store.isEnabled("test.zero-rollout") == false)
    }
        ...
        for _ in 0..<trials {
            ...
            let store = FeatureFlagStore(
                ...
                strategies: [
                    RolloutStrategy(
                        enrollment: enrollment,
                        rollouts: ["test.half": RolloutDefinition(percentage: 50)]
                    )
                ]
            )
            ...
            if store.isEnabled("test.half") { enabledCount += 1 }
        }
